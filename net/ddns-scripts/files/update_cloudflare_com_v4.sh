...
# Accept both the historical host@example.com syntax and the newer
# host.example.com form while remaining compatible with BusyBox ash.
case "$domain" in
	@.*) domain="@${domain#@.}" ;;
esac
case "$domain" in
...
__HOST="${domain%%@*}"
__DOMAIN="${domain#*@}"
[ -z "$__HOST" ] || [ "$__HOST" = "$__DOMAIN" ] && __HOST="@"
if [ "$__HOST" = "@" ]; then
